...
class TasksDaoTest {
    ...
    fun updateTaskAndGetById() = runBlockingTest {
        ...
        val updateTask = Task("titleUpdate","descriptionUpdate",true,task.id)
        database.taskDao().updateTask(updateTask)

        // 3. Check that when you get the task by its ID, it has the updated values.
        val loaded = database.taskDao().getTaskById(task.id)
        assertThat(loaded?.id, `is`(task.id))
        assertThat(loaded?.title, `is`("titleUpdate"))
        assertThat(loaded?.description, `is`("descriptionUpdate"))
